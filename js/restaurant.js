$(function () {
  ...
  function getTourList() {
    axios.get("https://ptx.transportdata.tw/MOTC/v2/Tourism/Restaurant?%24top=30&%24format=JSON", {
      ...
      .then(function (response) {
        ...
        thisData.forEach(function (item) {
          ...
          let sliceDesStr = `${item.Description}`.slice(0, 65);
  
          str += `
                <article class="col-12 col-md-6 tm-post">
                      <hr class="tm-hr-primary">
                      <a href="restaurant-page.html?id=${item.RestaurantID}" class="effect-lily tm-post-link tm-pt-60">
                          ...
                          <h2 class="tm-pt-30 tm-color-primary tm-post-title">${item.RestaurantName}</h2>
                      </a>                    
                      <p class="tm-pt-30">`
            + sliceDesStr + `<span>．．．</span>` +
            `<a href="restaurant-page.html?id=${item.RestaurantID}" class ="moreBtn">
                       more</a>`+
            `</p>
                      <div class="d-flex justify-content-between tm-pt-45">
                          <span class="tm-color-primary">活動地點 : ${item.Address}</span>
                          <span class="tm-color-primary"></span>
                      </div>
                      <hr>
                      <div class="d-flex justify-content-between">
                          <span>營業時間 : ${item.OpenTime}</span>
                          <span></span>
                      </div>
                  </article>`
  ...
  */
  ...
  sendBtn.addEventListener('click', function (e) {
    ...
    axios.get(
      `https://ptx.transportdata.tw/MOTC/v2/Tourism/Restaurant/${city}?$filter=contains(RestaurantName,'${keywords}')&$format=JSON`,
      {
        headers: getAuthorizationHeader()
      })
      .then(function(response){
       ...
       thisData.forEach(function (item) {
         ...
         let sliceDesStr = `${item.Description}`.slice(0, 68);
         //console.log(item);      
